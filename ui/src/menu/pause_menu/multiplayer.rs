...
use simgine_core::{error_event::trigger_error, network::{DEFAULT_PORT, Host, StopServer}, state::GameState};

use crate::widget::{button::style::ButtonStyle, dialog::{Dialog, DialogButton, DialogTitle}, text_edit::TextEdit, theme::{GAP, NORMAL_TEXT}};

pub(super) fn plugin(app: &mut App) {
    app.add_systems(PostUpdate, update_start_stop);
}

fn update_start_stop(mut text: Single<&mut Text, With<StartStopButton>>, server_state: Res<State<ServerState>>) {
        ...
        Children::spawn(SpawnWith(|parent: &mut RelatedSpawner<_>| {
            ...
            parent.spawn((
                ...
                Children::spawn(SpawnWith(|parent: &mut RelatedSpawner<_>| {
                    ...
                    let port_edit = parent.spawn((TextEdit, TextInputValue(DEFAULT_PORT.to_string()))).id();
                    let start_stop = move |_on: On<Pointer<Click>>, mut commands: Commands, server_state: Res<State<ServerState>>, edit_values: Query<&TextInputValue>| -> Result<()> {
                        match **server_state {
                            ServerState::Stopped => {
                                let port = edit_values.get(port_edit).unwrap();
                                let port: u16 = port.0.parse().map_err(|e| format!("invalid port {}: {e}", port.0))?;
                    ...
                    parent.spawn(StartStopButton).observe(start_stop.pipe(trigger_error));
                })),
            ));
